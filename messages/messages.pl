...
% Added by Miha Drole for handling negated literals

%msg(most_compressive_hypothesis, Verbosity, Necu) :- print(Necu), length(Necu, Len), print(Len), nl, fail.

msg(most_compressive_hypothesis, Verbosity, [Hypothesis, TestFold, PosEIDsCov, NegEIDsCov, _AuxClauses, PosEIDsRemain, NegEIDsRemain]):-
  Verbosity>0,
  format("The following highest scoring hypothesis was generated:~n", []),
  prettyPrintLiterals(Hypothesis),nl,
  print(PosEIDsCov), nl, print(NegEIDsCov), nl,
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Added by Miha Drole, Handles Negation
showHypothesis((Hypothesis, _HypSig, NumLiterals, WPos, WNeg, WNewPos, WNewNeg, AuxHypotheses)):-
...
msg(best_rmg, Verbosity, [Iter, Score, rlgg(core(SeedID, GenPosEIDs, Indexs), clause(Clause, ClauseSig)), PosIDsCov, NegIDsCov]):-
  !,
  (setting(theory_construction, global) -> Verb is Verbosity-2 ; Verb = Verbosity),
  (Verb>=2 ->
    length(Clause, NumLiterals),
