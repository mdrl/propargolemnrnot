...
% GILPS modules
:- use_module('../bottom clause/bottom_clause', [ground_sat/4, cacheExample/2, parsatcache/7]). %to compute the ground bottom clause from one example
...
load_example(ExampleAtom, Weight, ExampleID):-
  ...
  (setting(engine, propargolem) ->
       cacheExample(ExampleID, ExampleAtom),
       parsatcache(ExampleID, [ExampleAtom|ExampleGroundBodyBottom], _, _, _, _, _)
       ;
...
load_examples_aux([Example|Examples], EID, [EID|EIDs]):-
  Example=[Head|Body],
  (setting(engine, propargolem) ->
       cacheExample(ExampleAtom, ExampleID)
       ;
       true
  ),
