...
:- use_module('../bottom clause/bottom_clause', [sat/3, parallel_sat/4, parsatcache/8]).
:- use_module('../bottom clause/clause_reduce', [positive_clause_reduce/3, positive_clause_reduce_list/3, positiveClauseReduce/6, negative_clause_reduce/8, negative_clause_reduce/10, negative_clause_reduce_with_neg/8]).
...
:- use_module('../examples/examples', [id2example/2, exampleIDsWeights/2, invertExamples/1, ids2Examples/2]).
...
:- use_module('../utils/list', [elemsAtPos/3, firstNElemsOf/3, n_randomElems/3, randomPairs/3, custom_qsort/3, numbersList/3, nrandom_elements/3, random_tuples/4]).
...
% YAP modules
:- use_module(library(lists), [nth/3, nth/4, reverse/2, flatten/2, append/3, remove_duplicates/2]).
...
create_parsat(ExampleIDs, armg(core(ExampleID, [ExampleID], Indexs), clause(Bottom, BottomSig))):-
  ids2Examples(ExampleIDs, Examples),
  ExampleIDs = [ExampleID | _],
  parallel_sat(Examples, mode(variablized, default), Bottom, BottomSig),
  length(Bottom, NumLiterals),
  numbersList(1, NumLiterals, Indexs).
...
generate_seed_clauses(_, APosEIDs, TNegEIDs, SeedClauses, Depth):-
  ...
  setting(par_n_sat, PairSize),
  message(compute_rmg_pairs, []),
  random_tuples(NumPairs, PairSize, APosEIDs, Pairs),
...
rmg_pair_info(PosEIDs, NegEIDs, _, Pairs, (Score, NumLits, PosEIDsCov, NegEIDsCov, armg(Core, clause(Clause, ClauseSig), []))):-
  Pairs = [ExID1 | OtherExamples],
  id2example(ExID1, Example1),
  OtherExamples = [ExID2 | _],
  id2example(ExID2, Example2),
  (setting(progolem_refinement_operator, armg) ->
     create_parsat(Pairs, ARMG1), !,
     positive_clause_reduce_list(ARMG1, OtherExamples, armg(Core, clause(Clause, ClauseSig)))
  ;
...
%
%
% Asserts that a particular predicate is clp
%
%

:- dynamic(clp_predicates/1).

clp_predicate(X) :-
    assertz(clp_predicates(X)).
