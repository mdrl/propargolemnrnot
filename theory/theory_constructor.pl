...
:- use_module(library(ordsets), [ord_subtract/3, ord_union/3, list_to_ord_set/2]).

%clauses_to_theory(+Clauses, +ClauseNum, +PosExampleIDs, +NegExampleIDs, +CurTheory, -FinalTheory)
clauses_to_theory([], _, _, _, T, T).
clauses_to_theory([(Clause, ClauseSig)|Clauses], N, PosEIDsT, NegEIDsT, CurTheory, FTheory):-
  list_to_ord_set(PosEIDsT, PosEIDs),
  list_to_ord_set(NegEIDsT, NegEIDs),
...
:- meta_predicate construct_incremental_theory(?, ?, ?, ?, :, ?, ?, ?). % added by Miha Drole, added argument for negation depth
...
construct_incremental_theory(TPosEIDs, TNegEIDs, _APosEIDs, _ANegEIDs, HypGenPred, TestFold, Theory, Depth):-
  setting(max_clauses_per_theory, MaxClauses),
  emptyTheory(InitialTheory),
  %we should randomize the order of the PosExampleIDs ...
  construct_incremental_theory(TPosEIDs, TNegEIDs, TPosEIDs, 0, HypGenPred, 0, MaxClauses,
                               TestFold, InitialTheory, Theory, Depth).
...
construct_incremental_theory(TPosEIDs, TNegEIDs, APosEIDs, NUE, HypGenPred, NumClauses, MaxClauses,
  ...
  BestHypothesis=(_Score, Hypothesis, _HypSig, _NumLiterals, PosEIDsCovered, NegEIDsCovered, _),
  list_to_ord_set(PosEIDsCovered, PosEIDsCoveredOrd),
  list_to_ord_set(TPosEIDs, TPosEIDsOrd),
  list_to_ord_set(APosEIDs, APosEIDsOrd),
  ord_subtract(TPosEIDsOrd, PosEIDsCoveredOrd, NTPosEIDs), % this will have to be changed to minus/3 if/when PosEIDs is not ordered
% ord_subtract(TPosEIDs, PosEIDsCovered, NTPosEIDs), % this will have to be changed to minus/3 if/when PosEIDs is not ordered
%  is_ord(TPosEIDsOrd),
%  is_ord(PosEIDsCoveredOrd),
%  ord_subtract(APosEIDs, PosEIDsCovered, NAPosEIDs), % we have to remove the positive examples covered
  ord_subtract(APosEIDsOrd, PosEIDsCoveredOrd, NAPosEIDs), % we have to remove the positive examples covered
...
%
% Added by Miha Drole to account for depth of negated literals
%

construct_incremental_theory(_, _, _, _, _, MaxClauses, MaxClauses, _, Theory, Theory, Depth):-!.
construct_incremental_theory([], _, _, _, _, _, _, _, Theory, Theory, Depth):-!.
construct_incremental_theory(TPosEIDs, TNegEIDs, APosEIDs, NUE, HypGenPred, NumClauses, MaxClauses,
                             TestFold, CurrentTheory, Theory, Depth):-
  call_with_args(HypGenPred, TPosEIDs, TNegEIDs, APosEIDs, TestFold, BestHypothesis1, Depth),!,
  updateBestHypothesis(BestHypothesis1, BestHypothesis),%Signature must get here!!
  addHypothesisToTheory(CurrentTheory, TestFold, BestHypothesis, NCurrentTheory),
  BestHypothesis=(_Score, Hypothesis, _HypSig, _NumLiterals, PosEIDsCovered, NegEIDsCovered, _),
  list_to_ord_set(PosEIDsCovered, PosEIDsCoveredOrd),
...
%  ord_subtract(APosEIDs, PosEIDsCovered, NAPosEIDs), % we have to remove the positive examples covered
  ...
  NumClauses1 is NumClauses+1,
  construct_incremental_theory(NTPosEIDs, NTNegEIDs, NAPosEIDs, NUE, HypGenPred, NumClauses1,
                               MaxClauses, TestFold, NCurrentTheory, Theory, Depth).
...
construct_incremental_theory([TPosEID|TPosEIDs], TNegEIDs, APosEIDs, NUE, HypGenPred, NumClauses, MaxClauses,
  ...
  (early_stop(NUE1, TPosEIDs, APosEIDs) ->
   ...
   ;
     construct_incremental_theory(NTPosEIDs, TNegEIDs, APosEIDs, NUE1,
                                  HypGenPred, NumClauses, MaxClauses, TestFold, CurrentTheory, Theory, Depth)
  ).
...
% Added by Miha Drole for aux clauses support
updateBestHypothesis((FScore, FClause, FClauseSig, FNumLiterals, _FPosIDsCov, _FNegIDsCov, _AuxClauses),
                     (FScore, FClause, FClauseSig, FNumLiterals, FFPosIDsCov, FFNegIDsCov, _AuxClauses)):-
...
%
% Returns:
%    NewTheory : an update theory, resulted from adding Hypothesis to CurrentTheory (also a Theory tuple)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Added by Miha Drole to handle auxiliary predicates for negation.
addHypothesisToTheory((TotLiterals, PosCovered, NegCovered, Hypotheses),  %Current Theory
                      TestFold, % the test fold
                      (_Score, Hypothesis, HypSig, NumLiterals, HPosCov, HNegCov, AuxPreds),   %Hypothesis
                      (NTotLiterals, NPosCovered, NNegCovered, NHypotheses)):- % New theory
  NTotLiterals is TotLiterals+NumLiterals,
  ord_union(HPosCov, PosCovered, NPosCovered),
  ord_union(HNegCov, NegCovered, NNegCovered),
  ...
  exampleIDsWeights(NewPosCovered, TestFold, WNewPosHyp),
  exampleIDsWeights(NewNegCovered, TestFold, WNewNegHyp),
  append(Hypotheses, [(Hypothesis, HypSig, NumLiterals, WPosHyp, WNegHyp, WNewPosHyp, WNewNegHyp, AuxPreds)], NHypotheses).
