...
% YAP modules
:- use_module(library(lists), [member/2, append/3, nth/4, nth/3, select/3]).
...
%%%%%%%%%%%%%%%%%%%%%%%%%
%
% nrandom_elements( N, List, RandomSelected)
%
%%%%%%%%%%%%%%%%%%%%%%%%%

nrandom_elements(0, _, []) :- !.

nrandom_elements(_, [], []) :- !.

nrandom_elements(N, List, [RandEl|OtherEl]) :-
    length(List, LLen),
    random(1, LLen, Pos),
    nth(Pos, List, RandEl, Residual),
    N1 is N - 1,
    nrandom_elements(N1, Residual, OtherEl).

random_tuples(0, _, _, []) :- !.
random_tuples(NumTuples, TupleSize, List, [Tuple|Tuples]) :-
    nrandom_elements(TupleSize, List, Tuple),
    NewNumTuples is NumTuples - 1,
    random_tuples(NewNumTuples, TupleSize, List, Tuples).
