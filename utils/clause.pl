...
redundant(Lit, Lits, NLits):-
  ...
  member(Lit, NLits),
  simplfied_subsumption(Lits, TempLits).
...
hypothesisBody([H|T], T).

hypothesisHead([], []).

hypothesisHead([H|T], H).
...
split_pos_neg([\+CH|CT], [\+CSH|CST], PosClause, PosClauseSig, [\+CH | NegClause], [\+CSH | NegClauseSig]) :-
    !, split_pos_neg(CT, CST, PosClause, PosClauseSig, NegClause, NegClauseSig).

split_pos_neg([CH|CT], [CSH|CST], [CH|PosClause], [CSH|PosClauseSig], NegClause, NegClauseSig) :-
    split_pos_neg(CT, CST, PosClause, PosClauseSig, NegClause, NegClauseSig).


split_pos_neg([], [], []).

split_pos_neg([\+ CH | CT], PosClause, [\+ CH | NegClause]) :-
    !, split_pos_neg(CT, PosClause, NegClause).

split_pos_neg([CH|CT], [CH|PosClause], NegClause) :-
    !, split_pos_neg(CT, PosClause, NegClause).
